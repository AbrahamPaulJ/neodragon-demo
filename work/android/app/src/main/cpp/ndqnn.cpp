...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "ndqnn", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "ndqnn", __VA_ARGS__)
...
void qnnLog(const char* fmt, QnnLog_Level_t level, uint64_t, va_list argp) {
  ...
  int prio = level == QNN_LOG_LEVEL_ERROR ? ANDROID_LOG_ERROR
           : level == QNN_LOG_LEVEL_WARN  ? ANDROID_LOG_WARN
                                          : ANDROID_LOG_INFO;
  __android_log_print(prio, "ndqnn.qnn", "%s", buf);
}
...
Java_com_neodragon_demo_NativeQnn_lastError(JNIEnv* env, jclass) {
  return env->NewStringUTF(g_err.c_str());
}

JNIEXPORT jboolean JNICALL
Java_com_neodragon_demo_NativeQnn_init(JNIEnv* env, jclass, jstring jBackend, jstring jSystem,
                                       jstring jSkelDir) {
...
Java_com_neodragon_demo_NativeQnn_load(JNIEnv* env, jclass, jstring jPath) {
  std::lock_guard<std::mutex> lk(g_mu);
...
JNIEXPORT jstring JNICALL
Java_com_neodragon_demo_NativeQnn_describe(JNIEnv* env, jclass, jlong h) {
...
Java_com_neodragon_demo_NativeQnn_execute(JNIEnv* env, jclass, jlong h,
...
JNIEXPORT jobjectArray JNICALL
Java_com_neodragon_demo_NativeQnn_outputNames(JNIEnv* env, jclass, jlong h) {
  auto* m = (Model*)(intptr_t)h;
...
Java_com_neodragon_demo_NativeQnn_free(JNIEnv*, jclass, jlong h) {
  std::lock_guard<std::mutex> lk(g_mu);
  auto* m = (Model*)(intptr_t)h;
  if (!m) return;
