...
class Video(
    ...
    fun generate(prompt: String, seed: Long = 0L, image: Bitmap? = null,
        ...
        // vector above. clipg is NOT: `FirstFrame.generate()` is called ~30 lines below
        // and needs it again. Releasing it here made the video path map its 1.4 GB
        // context TWICE, at 1897 ms a time, for no benefit (measured 2026-08-23).
        ...
        // ---- 2. conditioning temb per stage -------------------------------------
        val temb = FloatArray(numStages * 1536).also { buf ->
            for (s in 0 until numStages) hostTemb(stageT[s], pooled).copyInto(buf, s * 1536)
        }

        // ---- 3. first frame -> unit 0 latent -------------------------------------
        val bmp: Bitmap
        if (image != null) {
        ...
        // Free the first-frame path UNCONDITIONALLY. These may be resident even in the
        // image-to-video case -- a previous "Image" run leaves ssd1bunet (1.3 GB), clipl
        // and ssd1bvaedec mapped, and clipg (1.4 GB) is deliberately kept. None are
        // needed past this point in either mode, and the MMDiT is about to map three
        // ~1.5 GB stages on top. Leaving these behind put ~7.5 GB of context in one
        // process and the app was killed mid-loop (observed 2026-08-23).
        //
        ...
        // moments = [mean | logvar] on the channel axis; the reference SAMPLES from the
        // distribution, so the video is not a deterministic function of the first frame.
        val nLat = latC * H * W
        ...
        for (unit in startUnit until numUnits) {
            ...
            for (stage in 0 until numStages) {
                ...
                val pred = runMmdit(c, stage, real, context, temb, stage, t5mask)
        ...
        val latents = Latent.catTime(history)
